# Fix Six Island Tilesets to Sevii Islands 67
# This script fixes ONLY Six Island layouts, nothing else

Write-Host "=== FIXING SIX ISLAND TILESETS ===" -ForegroundColor Green
Write-Host "Target: Change Six Island from Hoenn tilesets to gTileset_SeviiIslands67" -ForegroundColor Yellow

# Step 1: Create backup
Write-Host "Step 1: Creating backup..." -ForegroundColor Cyan
Copy-Item "data\layouts\layouts.json" "data\layouts\layouts_backup_six_island.json"
Write-Host "Backup created: layouts_backup_six_island.json" -ForegroundColor Green

# Step 2: Read the file
Write-Host "Step 2: Reading layouts.json..." -ForegroundColor Cyan
$lines = Get-Content "data\layouts\layouts.json"

# Step 3: Find Six Island layouts and fix them
Write-Host "Step 3: Finding and fixing Six Island layouts..." -ForegroundColor Cyan
...
for ($i = 0; $i -lt $lines.Count; $i++) {
    ...
    # Check if this is a Six Island layout ID
    if ($line -match '"id": "LAYOUT_SIX_ISLAND') {
        Write-Host "Found Six Island layout at line $($i+1): $line" -ForegroundColor Yellow
...
$verification = Get-Content "data\layouts\layouts.json" | Select-String -Pattern "LAYOUT_SIX_ISLAND" -Context 0,6
Write-Host "Verification result:" -ForegroundColor Yellow
Write-Host $verification -ForegroundColor White

Write-Host "=== SIX ISLAND FIX COMPLETE ===" -ForegroundColor Green
Write-Host "Six Island now uses gTileset_SeviiIslands67" -ForegroundColor Green
