# Fix Five Island Tilesets to Sevii Islands 45
# This script fixes ONLY Five Island layouts, nothing else

Write-Host "=== FIXING FIVE ISLAND TILESETS ===" -ForegroundColor Green
Write-Host "Target: Change Five Island from Hoenn tilesets to gTileset_SeviiIslands45" -ForegroundColor Yellow

# Step 1: Create backup
Write-Host "Step 1: Creating backup..." -ForegroundColor Cyan
Copy-Item "data\layouts\layouts.json" "data\layouts\layouts_backup_five_island.json"
Write-Host "Backup created: layouts_backup_five_island.json" -ForegroundColor Green

# Step 2: Read the file
Write-Host "Step 2: Reading layouts.json..." -ForegroundColor Cyan
$lines = Get-Content "data\layouts\layouts.json"

# Step 3: Find Five Island layouts and fix them
Write-Host "Step 3: Finding and fixing Five Island layouts..." -ForegroundColor Cyan
...
for ($i = 0; $i -lt $lines.Count; $i++) {
    ...
    # Check if this is a Five Island layout ID
    if ($line -match '"id": "LAYOUT_FIVE_ISLAND') {
        Write-Host "Found Five Island layout at line $($i+1): $line" -ForegroundColor Yellow
...
$verification = Get-Content "data\layouts\layouts.json" | Select-String -Pattern "LAYOUT_FIVE_ISLAND" -Context 0,6
Write-Host "Verification result:" -ForegroundColor Yellow
Write-Host $verification -ForegroundColor White

Write-Host "=== FIVE ISLAND FIX COMPLETE ===" -ForegroundColor Green
Write-Host "Five Island now uses gTileset_SeviiIslands45" -ForegroundColor Green
